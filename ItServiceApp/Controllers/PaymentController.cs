...
namespace ItServiceApp.Controllers
{
    public class PaymentController : Controller
    {
        ...
        [Authorize]
        public IActionResult Purchase(Guid id)//satın alma için get işlemi
        {
            ...
            var model2 = new PaymentViewModel()
            {
                BasketModel = new BasketModel()
                {
                    ...
                    Id = data.Id.ToString(),
                    Name = data.Name,
                    Price = data.Price.ToString(new CultureInfo("en-us"))//Global para birimi aslında yukarda setlemiştik
                }
            };
            return View(model2);
        }
        ...
        public async Task<IActionResult> Purchase(PaymentViewModel model )
        {
            ...
            var basketModel = new BasketModel()
            {
                ...
                Id = type.Id.ToString(),
                Name = type.Name,
                Price = type.Price.ToString(new CultureInfo("en-us"))//Global para birimi aslında yukarda setlemiştik
            };
            var data = _dbContext.SubscriptionTypes.Find(Guid.Parse(model.BasketModel.Id));


            var model1 = _mapper.Map<SubscriptionTypeViewModel>(data);
            ViewBag.Subs = model1;
            ...
            //Adresi bulmak için
            var address = _dbContext.Addresses.Include(x=>x.State.City).First(x=>x.Id==Guid.Parse(model.AddressModel.Id));
